...
impl FailInfo {
    fn description(&self) -> String {
        ...
        match self {
            ...
            InvalidAsgn(name)   => format!("{} '{}' {}", "Assignment".red(), name.to_string_lossy(), "is invalid or non-existant.".red()),
            InvalidUser(name)   => format!("{} '{}' {}", "User name".red(), name.to_string_lossy(), "is invalid or non-existant".red()),
            InvalidCWD()        => format!("{}", "Current working directory is invalid.".red()),
            InvalidUID()        => format!("{}", "User identifier invalid.".red()),
            LocalBuildFail(err) => format!("{}\n\n{}", "Build failure in current working directory:".red(), err.to_string_lossy()),
            DestBuildFail(err)  => format!("{}\n\n{}", "Build failure in submission directory:".red(), err.to_string_lossy()),
            FormatFail(err)     => format!("{}\n\n{}", "Failed to format files. Error:".red(), err.to_string_lossy()),
            StyleFail(err)      => format!("{}\n\n{}", "Failed to check style. Error:".red(), err.to_string_lossy()),
            TestFail(err)       => format!("{}\n\n{}", "Failed to test functionality due to internal error. Error:".red(), err.to_string_lossy()),
            MissingFile(name)   => format!("{} '{}' {}", "File".red(), name.to_string_lossy(), "does not exist in current working directory.".red()),
            MissingSub(name)    => format!("{} '{}' {}", "File".red(), name.to_string_lossy(), "does not exist in the submission directory".red()),
            FileIsDir(name)     => format!("{} '{}' {}", "File".red(), name.to_string_lossy(), "is actually a directory".red(),),
            FileIsOther(name)   => format!("{} '{}' {}", "File".red(), name.to_string_lossy(), "in neither a file nor a directory".red()),
            NoSetup(name)       => format!("{} '{}'", "Setup files are not available for assignment".red(), name.to_string_lossy()),
    ...
    fn advice(&self) -> String {
        ...
        match self {
            ...
            InvalidAsgn(name) => format!("{} '{}' {}", "If you believe".yellow(), name.to_string_lossy(), "is a valid assignment name, please contact the instructor.".yellow()),
            InvalidUser(name) => format!("{} '{}' {}", "If you believe".yellow(), name.to_string_lossy(), "is a valid user name, please contact the instructor.".yellow()),
            ...
            MissingFile(name) => format!("{} '{}' {}", "Please ensure".yellow(), name.to_string_lossy(), "is an existing file in your directory.".yellow()),
            MissingSub(name)  => format!("{} '{}' {}", "File", name.to_string_lossy(), "cannot be recovered.".yellow()),
            FileIsDir(name)   => format!("{} '{}' {}", "Please ensure that".yellow(), name.to_string_lossy(), "is a file.".yellow()),
            FileIsOther(name) => format!("{} '{}' {}", "Please ensure that".yellow(), name.to_string_lossy(), "is truely a file.".yellow()),
            NoSetup(name)     => format!("{} '{}' {}", "If you believe assignment".yellow(), name.to_string_lossy(), "should have setup files, please contact the instructor".yellow()),
